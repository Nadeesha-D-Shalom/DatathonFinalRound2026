...
class BackendTests(unittest.TestCase):
    ...
    def test_supervisor_routes_all_tasks(self):
        ...
        self.assertEqual(supervisor.run('compare_2030_scenario', {'country': 'Germany', 'target_year': 2030, 'energy_mix': VALID_MIX})['status'], 'model_not_connected')
    ...
    def test_api_development_states(self):
        ...
        self.assertEqual(self.client.post('/api/scenario/compare', json={'country': 'Germany', 'target_year': 2030, 'energy_mix': VALID_MIX}).json()['status'], 'model_not_connected')
        self.assertEqual(self.client.post('/api/scenario/compare', json={'country': 'Egypt', 'target_year': 2028, 'energy_mix': VALID_MIX}).json()['status'], 'model_not_connected')

    def test_browser_preflight_and_scenario_response_are_distinct(self):
        preflight = self.client.options('/api/scenario/compare', headers={
        ...
        response = self.client.post('/api/scenario/compare', json={
            'country': 'Egypt', 'target_year': 2027, 'energy_mix': VALID_MIX,
    ...
    def test_scenario_year_contract_and_mix_tolerance(self):
        for year in (2027, 2028, 2029, 2030):
            response = self.client.post('/api/scenario/compare', json={'country': 'Egypt', 'target_year': year, 'energy_mix': VALID_MIX})
        ...
        response = self.client.post('/api/scenario/compare', json={'country': 'Egypt', 'target_year': 2028, 'energy_mix': near})
        self.assertEqual(response.json()['status'], 'model_not_connected')
        invalid = {**VALID_MIX, 'coal_pct': 25}
        response = self.client.post('/api/scenario/compare', json={'country': 'Egypt', 'target_year': 2028, 'energy_mix': invalid})
    ...
    def test_country_list_and_profile_are_csv_backed(self):
        ...
        self.assertEqual(profile['latest_year'], max(row['year'] for row in profile['history']['co2']))
        self.assertEqual(profile['latest_year'], max(row['year'] for row in profile['history']['energy_mix']))
        root = Path(__file__).resolve().parents[2] / 'Dataset'
        with (root / 'co2_emissions_yearly.csv').open(newline='', encoding='utf-8') as source:
            all_co2 = [row for row in csv.DictReader(source) if row['country'] == 'Algeria' and 2000 <= int(row['year']) <= 2026]
            original_co2 = next(row for row in all_co2 if int(row['year']) == profile['latest_year'])
        with (root / 'energy_mix_yearly.csv').open(newline='', encoding='utf-8') as source:
            all_energy = [row for row in csv.DictReader(source) if row['country'] == 'Algeria' and 2000 <= int(row['year']) <= 2026]
            original_energy = next(row for row in all_energy if int(row['year']) == profile['latest_year'])
        self.assertEqual(profile['co2']['co2_per_capita_t'], float(original_co2['co2_per_capita_t']))
        self.assertEqual(profile['energy_mix']['renewables_total_pct'], float(original_energy['renewables_total_pct']))
        self.assertEqual([row['year'] for row in profile['history']['co2']], sorted(int(row['year']) for row in all_co2))
        self.assertEqual([row['year'] for row in profile['history']['energy_mix']], sorted(int(row['year']) for row in all_energy))
    ...
    def test_scenario_uses_same_agent_twice(self):
        ...
        agent = FakeAgent()
        engine = ScenarioEngine(agent, lambda country, year: {'status': 'success', 'energy_mix': VALID_MIX})
    ...
    def test_invalid_annual_model_output_is_rejected(self):
        ...
        engine = ScenarioEngine(InvalidAnnualAgent(), lambda country, year: {'status': 'success', 'energy_mix': VALID_MIX})
    ...
    def test_valid_annual_forecast_passes_through_without_interpolation(self):
        ...
        engine = ScenarioEngine(AnnualAgent(), lambda country, year: {'status': 'success', 'energy_mix': VALID_MIX})
    ...
    def test_selected_year_uses_only_validated_model_year(self):
        ...
        class AnnualAgent:
            ...
            def predict(self, energy_mix, target_year=None):
                ...
                annual = [{'year': year, 'co2_per_capita_t': year - 2020 + offset, 'co2_emissions_mt': 100 + year - 2027 + offset} for year in range(2027, 2031)]
                return {'status': 'success', 'co2_per_capita_t': annual[-1]['co2_per_capita_t'], 'yearly_forecast': annual, 'model': {'name': 'test', 'r2': 0.5, 'rmse': 1}}
        engine = ScenarioEngine(AnnualAgent(), lambda country, year: {'status': 'success', 'energy_mix': VALID_MIX})
        request = {'country': 'Egypt', 'target_year': 2028, 'energy_mix': {**VALID_MIX, 'wind_pct': 30, 'solar_pct': 5}}
    ...
    def test_missing_annual_result_cannot_be_reused_for_2028(self):
        class EndpointAgent:
            connected = True
            def predict(self, energy_mix, target_year=None):
                return {'status': 'success', 'co2_per_capita_t': 7, 'model': {'name': 'test', 'r2': 0.5, 'rmse': 1}}
        engine = ScenarioEngine(EndpointAgent(), lambda country, year: {'status': 'success', 'energy_mix': VALID_MIX})
        response = Supervisor(scenario_engine=engine).run('compare_2030_scenario', {'country': 'Egypt', 'target_year': 2028, 'energy_mix': VALID_MIX})
