...
const paths = Object.fromEntries(['Business-as-Usual', 'Moderate Transition', 'Accelerated Transition'].map(name => [name, {
  forecast: [2026, 2027, 2028, 2029, 2030].map(year => ({ year, co2_emissions_mt: year - 1900 })),
}]))
const rows = [{ year: 2026, co2_emissions_mt: 100 }]
assert.equal(buildTotalEmissionsChart(rows, paths, null).find(row => row.year === 2028).plan, undefined)

const mix = { coal_pct: 15, oil_pct: 10, gas_pct: 20, nuclear_pct: 10, hydro_pct: 5, solar_pct: 20, wind_pct: 15, other_renewables_pct: 5 }
const result = {
  status: 'success', country: 'Egypt', target_year: 2028,
  baseline: { label: 'Current Trend', energy_mix: mix, co2_per_capita_t: 8 },
  user_scenario: { label: 'Your Energy Plan', energy_mix: mix, co2_per_capita_t: 6, co2_emissions_mt: 111 },
  comparison: { baseline: 8, scenario: 6, difference: -2, reduction: 2, reduction_percent: 25, direction: 'decrease' },
...
annualResult.user_scenario.yearly_forecast = [2027, 2028, 2029, 2030].map(year => ({ year, co2_per_capita_t: year - 2020, co2_emissions_mt: year - 1900 }))
...
const { compareScenario } = load('lib/api/client.ts', {
  ...
  fetch: async (url, options) => {
    ...
    return { ok: true, json: async () => ({ status: 'model_not_connected', message: 'Awaiting model' }) }
  },
})
;(async () => {
