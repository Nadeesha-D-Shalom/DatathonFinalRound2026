...
export function rebalanceEnergyMix(
  current: EnergyMix,
  edited: keyof EnergyMix,
  input: number,
): EnergyMix {
  ...
  const weights = others.map((key) =>
    Number.isFinite(current[key]) ? Math.max(0, current[key]) : 0,
  )
  ...
  others.forEach((key, index) => {
    ...
    const share = Math.max(
      0,
      Math.min(remainingCents - assigned, Math.round((remainingCents * weight) / divisor)),
    )
...
export function balanceEnergyMix(current: EnergyMix): EnergyMix {
  const weights = energyKeys.map((key) =>
    Number.isFinite(current[key]) ? Math.max(0, current[key]) : 0,
  )
  ...
  energyKeys.slice(0, -1).forEach((key, index) => {
    const share = Math.max(
      0,
      Math.min(10000 - assigned, Math.round((10000 * weights[index]) / total)),
    )
    output[key] = share / 100
    assigned += share
  })
