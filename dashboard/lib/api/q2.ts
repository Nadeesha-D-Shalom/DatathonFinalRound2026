...
export type Q2Summary = {
  ...
  experiment: {
    ...
    comparison: {
      ...
      event_window_improvement_percent: number
    }
  }
  conclusion: string
  source: string[]
}
export type Q2Features = {
  status: 'success'
  metric: string
  features: { feature: string; coefficient: number; abs_coefficient: number; description: string }[]
  source: string
}
export type Q2Window = {
  status: 'success'
  event_window: {
    rows: number
    baseline_rmse: number
    event_aware_rmse: number
    improvement_percent: number
  }
  source: string
}
export type Q2Impact = {
  status: 'success'
  count: number
  results: {
    date: string
    ...
    event_aware_prediction: number
    event_active_30d: number
  }[]
  source: string
}
