...
export function localCountryProfile(data: DashboardData, country: string): CountryEnergyCO2 | null {
  ...
  return {
    ...
    energy_mix: {
      ...
      fossil_total_pct: latest.fossil_total_pct,
    },
    history: {
      co2: rows.map((row) => ({
        year: row.year,
      ...
      energy_mix: rows.map((row) => ({
        year: row.year,
        renewables_total_pct: row.renewables_total_pct,
        fossil_total_pct: row.fossil_total_pct,
      })),
    },
