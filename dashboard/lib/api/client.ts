...
export type ScenarioSuccess = {
  ...
  baseline: {
    ...
    yearly_forecast?:
      { year: ForecastYear; co2_per_capita_t: number; co2_emissions_mt?: number | null }[] | null
  }
  user_scenario: {
    label: string
    energy_mix: EnergyMix
...
export class BackendUnavailableError extends Error {
  constructor() {
    super('Prediction service is offline. Start the FastAPI backend to enable model predictions.')
...
export async function request<T>(path: string, body?: unknown): Promise<T> {
  ...
  if (!result || typeof result !== 'object')
    throw new Error('Prediction service returned an unreadable response.')
  if (!response.ok && result.status !== 'validation_error')
    throw new Error(result.message || 'Prediction request failed.')
  return result as T
}
...
export const getBackendHealth = () =>
  request<{
    status: 'ok'
    carbon_model: 'connected' | 'not_connected'
    co2_model: 'connected' | 'not_connected'
  }>('/health')
export const getCarbonForecast = (market: string) =>
  request<CarbonForecastSuccess | ApiState>('/api/carbon/forecast', { market })
export type CarbonAnalysisOutlook = {
  status: 'success'
  market: string
  currency: string
  last_observed_date: string
...
export const getCarbonAnalysisOutlook = (market: string) =>
  request<
...
export const getCountryEnergyCO2 = (country: string) =>
  request<CountryEnergyCO2>(`/api/countries/${encodeURIComponent(country)}/energy-co2`)
export const compareScenario = (
  country: string,
  energyMix: EnergyMix,
  targetYear: ForecastYear = 2030,
) =>
  request<ScenarioSuccess | ApiState>('/api/scenario/compare', {
    country,
