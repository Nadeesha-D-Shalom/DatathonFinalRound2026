...
export function normalizeText(input: string): string {
  let text = input.toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[’']/g,' ').replace(/[‐‑–—]/g,'-')
  for (const [pattern,replacement] of substitutions) text = text.replace(pattern,replacement)
  return text.replace(/[^a-z0-9%+\- ]/g,' ').replace(/\s+/g,' ').trim()
}
export function containsPhrase(text: string, phrase: string): boolean {return new RegExp(`(?:^| )${phrase.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}(?: |$)`).test(text)}
